            ...
            <div style={{flex: '1', marginTop:'40px', marginLeft:'40px', marginRight:'40px'}}>
              <div>
                <div>
                  {isMobile && 
                  <div>
                    <div className={style.head} style={{fontSize: isDesktopOrLaptop ? '25px' : '20px', marginBottom:'10px' }}>질문 글 목록</div>
                    <Searchbar defaultSearchText="제목 및 내용으로 검색" onSearch={handleSearch}/>
                  </div>}
                  {!isMobile && <div className={style.background_head}>
                    <div className={style.head} style={{fontSize: isDesktopOrLaptop ? '25px' : '20px' }}>질문 글 목록</div>
                    <Searchbar defaultSearchText="제목 및 내용으로 검색" onSearch={handleSearch}/>
                  </div>}
                ...
                {/* 정렬된 데이터를 표시 */}
                {sortedPosts.filter(post => post.category === '질문').map((post, index) => (
                  <Post
                    key={post.id}
                    id={post.id}
                    num={index + 1}
