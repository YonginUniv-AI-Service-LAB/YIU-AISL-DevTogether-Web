...
const postViewsState = (postId) => atom({
  key: `postViewsState_${postId}`,
  default: data_board.find(post => post.id === postId)?.views || 0,
});

const postViewState = (postId) => atom({
  key: `postViewState_${postId}`,
  default: false, // 각 포스트의 조회 상태는 기본적으로 false로 설정합니다.
});

const postCommentsState = (postId) => atom({
    key: `postCommentsState_${postId}`,
    default: data_comment.filter(comment => comment.post_id === postId).length || 0,
});
